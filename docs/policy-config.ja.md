...
このファイルが、AI に見せない場所、`.gitignore` 候補、
暗号化対象、コミットブロック対象を決める唯一の入力です。

このページは、機密パスを追加するときに
どの項目を書けばどの生成物や検査に反映されるかを確認するためのものです。
...
- `aiignore: true`:
  ...
  Claude `denyRead` などの AI ツールやエディタ向け除外出力に含める
- `gitignore: true`:
...
- `aiignore: true`: AI 向け除外ファイルと Claude Code 設定例に出る
- `gitignore: true`: `.gitignore.ai-sensitive-files` に出る
- `encrypt: true`: 暗号化計画と平文チェックの対象になる
- `commit_block: true`: Git 管理されていると `check` が失敗する
...
| `decrypted_path` | `/workspaces/.runtime-secrets/app.env` | Dev Container が読むワークスペース外の環境変数ファイル |
