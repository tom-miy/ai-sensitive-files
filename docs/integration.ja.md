...
このページは、適用先リポジトリに何を置き、
何を手動で確認するかを決めるための手順です。
...
単一の除外ファイルだけを唯一の制御にしないでください。

導入後に確認すること:

- `.ai-sensitive-files/sensitive-files.yaml` に実際の機密パスが書かれている
- `.gitignore.ai-sensitive-files` の内容を見てから `.gitignore` に取り込んでいる
- Lefthook を使う場合は、既存設定に手動で追記している
- SOPS/age や 1Password / Bitwarden の鍵・認証は別途用意している
