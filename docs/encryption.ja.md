...
このページは、平文ファイルと暗号化済みファイルの関係を
ポリシーにどう書くかを決めるためのものです。

`encrypt: true` は、その項目を暗号化計画と
平文チェックの対象にするという意味です。
このツールが暗号化そのものを実行するわけではありません。
どのコマンドで暗号化・復号するかを記録し、
コミット前に平文の残存や同期ずれを検査します。
...
実際の受信者一覧は SOPS の `.sops.yaml` にも保存されます。
ポリシー側にも同じ一覧を持たせることで、
...
秘密鍵管理を行いません。
`templates/sops/.sops.yaml.example` の受信者はダミーです。
...
- `.sops.yaml` やポリシーレビューに必要な
  age 公開鍵の受信者一覧は Git 管理してよい
- チームメンバーや CI が復号する必要がある場合は、
  共有前に必要な age 公開鍵をすべて受信者一覧に追加する
- 暗号化済みファイルをコミットする場合は、
...
- リポジトリ内に暗号化済みファイルを置くなら `encrypted_path` を書く
- 1Password / Bitwarden を定義元にするなら `crypto.secret_ref` を書く
- ローカル平文を手編集してよいなら `manual_edit: decrypted` にする
- 平文を残してはいけないなら `manual_edit: none` にする
